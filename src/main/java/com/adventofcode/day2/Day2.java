 ...
 * and moving to adjacent buttons on the keypad: U moves up, D moves down, L moves left, and R moves right. Each
 ...
 * button is 1. Starting from the previous button ("1"), you move right twice (to "3") and then down three times
...
public class Day2 {
    ...
    public static String solvePart2(List<String> input) {
        ...
        return solve(new Keypad(keypad), new Keypad.Position(2, 0), input);
    }

    private static String solve(Keypad keypad, Keypad.Position startPos, List<String> input) {
        Keypad.Position curPos = new Keypad.Position(startPos.i, startPos.j);

        String code = "";

        for (String line : input) {
            curPos = next(keypad, curPos, parseCommands(line));
            code += keypad.at(curPos);
        }

        return code;
    }

    private static Stream<Command> parseCommands(String commands) {
        return commands.chars().mapToObj(command -> Command.parse((char) command));
    }

    private static Keypad.Position next(Keypad keypad, Keypad.Position current, Stream<Command> commands) {
        Keypad.Position next = current;

        for (Command command : commands.collect(toList())) {
            if (keypad.isValidCommand(command, next)) {
                next = command.nextPosition(next);
            }
        }
